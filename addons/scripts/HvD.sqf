...
for "_i" from 0 to 1 step 0 do 
{
	...
	_totaldrugs = _lsdInv+_marInv+_cocInv+_herInv;
	...
	if (isNil "createDrugsMarker" && _totaldrugs > 5) then
		{
			...
			createDrugsMarker = 
			{
				...
				_drugMarker setMarkerText (format ["Priority Target: %1", name player]);
