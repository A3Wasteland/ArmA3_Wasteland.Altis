...
aActionsIDs = aActionsIDs + [player addAction["<img image='\a3\Ui_f\data\GUI\Cfg\CommunicationMenu\attack_ca.paa'/> Open gun store", "[] spawn loadGunStore;", [], 1, false, false, "", '(vehicle player == player) && player distance (nearestobjects [player, ["C_man_1_1_F"],  3] select 0) < 2']];
aActionsIDs = aActionsIDs + [player addAction["<img image='\a3\Ui_f\data\GUI\Cfg\CommunicationMenu\defend_ca.paa'/> Open general store", "[] spawn loadGeneralStore;", [], 1, false, false, "", '(vehicle player == player) && player distance (nearestobjects [player, ["C_man_polo_6_F"],  3] select 0) < 2']];

if ((call config_player_saving_enabled) == 1) then {
	aActionsIDs = aActionsIDs + [player addAction["<img image=""client\icons\save.paa""/> <t color=""#3f71d7"">Save Player</t>", "persistence\players\c_savePlayerToServer.sqf", [], 1, false, false, "", 'stance player == "PRONE"']];
};
